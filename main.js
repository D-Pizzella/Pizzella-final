...
function compararTiempos() {
    ...
    if (isNaN(tiempoUsuario)) {
        Swal.fire({
            title: "Error",
            text: "Por favor, ingresa un valor numérico para el tiempo.",
        });
        return;
    }
    ...
    if (tiemposEnRango.length === 0) {
        Swal.fire({
            title: "Lo lamentamos...",
            text: "No se encontraron coincidencias en el rango especificado.",
        });
    } else {
...
function agregarTiempo() {
    ...
    agregarButton.addEventListener('click', function() {
        ...
        if (!isNaN(parseInt(nombre)) || !isNaN(parseInt(nacionalidad))) {
            Swal.fire({
                title: "Error",
                text: "Nombre y nacionalidad no pueden ser números.",
            });
            return;
        }
        ...
        if (isNaN(parseInt(edad)) || isNaN(parseFloat(nuevoTiempo))) {
            Swal.fire({
                title: "Error",
                text: "Por favor, ingresa valores numéricos para la edad y el tiempo.",
            });
            return;
        }
        ...
        if (!nacionalidad) {
            Swal.fire({
                title: "Error",
                text: "La nacionalidad no puede ser vacía. Por favor, ingrésela nuevamente.",
            });
            return;
        }
        ...
        if (!nombre) {
            Swal.fire({
                title: "Error",
                text: "El nombre no puede ser vacío. Por favor, ingréselo nuevamente.",
            });
            return;
        }
...
function agregarTiempoFromInput(nombre, nacionalidad, edad, nuevoTiempo) {
    ...
    Swal.fire({
        title: "El tiempo fue registrado.",
        text: "Ya podes compararlo",
    })
